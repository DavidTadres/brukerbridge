...
# David PC
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
host_IP = 'ENTER IP OF RIPPING COMPUTER USING LIKE THIS ->' #"171.65.16.149" # < change this to point to your own computer
user_json = 'name of user.json in folder "users"'
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
