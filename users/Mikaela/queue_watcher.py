...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
log_folder = pathlib.Path('PLACEHOLDER_LOG_PATH')
root_directory = pathlib.Path("PLACEHOLDER_RIPPING_DIRECTORY")
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
def launch_main_processing(dir_to_process, stripped_folder, log_folder):
	...
	exit_status = subprocess.call(['python', '-u', str(main_py_path), dir_to_process],stdout=f,stderr=f)
